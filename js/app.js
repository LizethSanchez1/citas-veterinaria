...
class UI {
    ...
    imprimirCitas({ citas }) {
        ...
        citas.forEach(cita => {
            ...
            btnEliminar=window.confirm("Eliminar")
            ///no me salio el Alert
            btnEliminar.addEventListener('click', function(eliminar){
                const eliminado = window.confirm('¿eliminar?');
                if (eliminado == true) {
                    const btnEliminar = document.createElement('button')
...
// //contenedor para citas //para id se tiene que usar #
// const contenedorCitas=document.querySelector('#citas')
...
// class Cita{
//     constructor(){
//         this.cita = [] //deja de estar vacio y contiene la primer cita agregado por el metodo agregarcita 
//     }
//     //agregarcita, eliminarcita, editarcita 
//     agregarCita(cita){ //entrada-proceso-salida //..//cita=nombre de mascota, nombre de propietario....sintomas
//         this.citas=[...this.citas, cita] //arry vacio de cita=array vacio de cita +cita
//         //cita recoje toda la info
//     }
//     editarCita(citaActualizada){
//         this.citas=this.citas.map(cita=>cita.id===citaActualizada.id ? citaActualizada : cita)
//         //map arroja un  nuevo arreglo con la info que se recorre
...
//         */
//     }
//     eliminarCita(id){ //eliminar por id //filter: Crea un nuevo arreglo, pero con todos los elementos que cumplan una cierta condicion
//         this.cita=this.citas.filter(citas=> this.cita.id!==id)
//         //1-2-3-4...24-26
//     }
...
//             const divCita=document.createElement('div')
//             divCita.classList.add('cita')
//             divCita.dataset.id=id//se le asigna el id
...
//             const propietarioParrafo = document.createElement('p')
//             mascotaParrafo.innerHTML=`<span class=''font-weight-bolder> Propietario: </span>${propietario}` 
...
//             const sintomasParrafo = document.createElement('p')
//             mascotaParrafo.innerHTML=`<span class=''font-weight-bolder> Propietario: </span>${sintomas}`
//             //agregando al HTML
//             divCita.appendChild(mascotaParrafo)
//             divCita.appendChild(propietarioParrafo)
//             divCita.appendChild(telefonoParrafo)
...
// }
// const ui= new UI(); //PARA UNA VARIABLE GLOBAL
// const adomCitas = new Cita()

// function nuevaCita(e){ //generando cita
//     e.preventDefault()

//     const {macota, propietario, telefono, fecha, hora, sintomas }= citaObject//objet destructuring
...
//     if (macota===''|| propietario === '' || telefono==='' || fecha ==='' || hora === '' || sintomas ==='') {
//         ui.imprimirAlerta('Todos los campos son obligatorios', 'error')
...
//     if(modificar){
//         //estamos editando cita
//         adomCitas.editarCita({...citaObject})
...
//         //generar ID
//         citaObject.id=Date.now //123456789128464
//         adomCitas.agregarCita({...citaObject})

//         ui.imprimirAlerta('Cita agregada correctamente')
//     }
//     //imprimiend el HTML de citas
//     ui.imprimirCitas(adomCitas)
//     reiniciarForm()

// }
// function eliminarCitas(id){ //25 --pasa el id a 
//     adomCitas.eliminarCita(id)//25
//     ui.imprimirCitas(adomCitas)
//     //para borrar reinicia el objeto para evitar futuros problemas de validacion

//     reiniciarObjeto()
//     //deja vacios los campos de texto
//     formulario.reset()

// }
// //para que se borre lo ingresado en el formulario sin necesidad de dar refresh
